...
def test_verify_uses_proof_manifest_object_format(
    tmp_path: Path,
    capsys: pytest.CaptureFixture[str],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Historical proofs remain verifiable across Git object formats."""
    ...
    historical_id = "09176d4f7849cf4cd19224e97d0e057b668cb7d2"
    payload = build_payload("sha1", historical_id)
    proof_bytes = _make_bitcoin_attested_proof(payload)
    _write_proof_and_manifest(repo, historical_id, proof_bytes)
    _write_config(repo, _fresh_config())
    calls: list[tuple[bytes, bytes]] = []
    ...
    class FakeClient:
        ...
        def verify(self, proof: bytes, subject: bytes) -> VerificationAttempt:
            calls.append((proof, subject))
            return VerificationAttempt(verified=True, detail="verified")

    monkeypatch.setattr("git_ots.cli.OpenTimestampsCli", FakeClient)

    main(["verify"], cwd=repo)

    assert calls == [(proof_bytes, payload)]
    assert "commit id must be 64 characters" not in capsys.readouterr().out
