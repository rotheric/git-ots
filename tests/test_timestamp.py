...
def test_process_group_cleanup_tolerates_macos_permission_race(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A redundant macOS kill may return EPERM after another cleanup wins."""

    def _permission_denied(_pgid: int, _signal: int) -> None:
        raise PermissionError(1, "Operation not permitted")

    monkeypatch.setattr("git_ots.timestamp.os.killpg", _permission_denied)
